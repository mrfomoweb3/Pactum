...
import { addStaff, closeBond, createPassToken, createPaymentIntent, createRefundIntent, createRegistrationNonce, createRestaurantBond, createTransferNonce, getAuditEvents, getPublicBond, getRestaurantBonds, getStaff, registerProfile, revokeStaff, storedProfile, submitTransfer, updateServiceStatus, validatePass, verifyPayment, verifyRefund, type AuditEvent, type PassToken, type PassValidation, type PaymentIntent, type Profile, type PublicBond, type RestaurantBond, type Role, type StaffMember } from './api'
...
function RestaurantHome() {
  ...
  async function close(bond: RestaurantBond, action: 'cancel' | 'forfeit') { if (!confirm(`${action} this reservation?`)) return; try { await closeBond(bond.public_id, action); setBonds(current => current.map(item => item.id === bond.id ? { ...item, terminal_status: action.toUpperCase() } : item)) } catch (caught) { setError(caught instanceof Error ? caught.message : 'Could not update reservation.') } }
  return <div className="workspace"><header><Mark/><span>{profile.displayName}</span></header><main><p className="eyebrow">Restaurant workspace</p><h1>Good evening,<br/>{profile.displayName}.</h1><div className="workspace-actions"><Link className="button button--app" to="/restaurant/new">Create reservation bond <ArrowRight/></Link><Link className="button workspace__secondary" to="/staff/scan"><QrCode/> Validate a guest pass</Link><Link className="button workspace__secondary" to="/restaurant/staff"><UserCircle/> Manage staff</Link></div>{error && <div className="error-message">{error}</div>}<div className="bond-list">{bonds.length ? bonds.map(bond => <section className="workspace-card" key={bond.id}><span>{bond.terminal_status || bond.service_status || bond.status}</span><h2>{new Date(bond.reservation_at).toLocaleString()} · {bond.party_size} guests</h2><p>{(bond.amount_luna / 100000).toFixed(2)} NIM bond</p><div className="bond-row-actions"><Link className="text-link bond-open" to={`/p/${bond.public_id}`}>Open reservation <ArrowRight/></Link>{bond.status === 'SECURED' && !bond.service_status && !bond.terminal_status && <><button className="refund-link" disabled={busyBond === bond.id} onClick={() => void refund(bond)}>{busyBond === bond.id ? 'Confirming refund…' : 'Refund bond'}</button><button className="refund-link" onClick={() => void close(bond, 'forfeit')}>Forfeit</button></>}{!bond.service_status && !bond.terminal_status && <button className="refund-link" onClick={() => void close(bond, 'cancel')}>Cancel</button>}</div></section>) : <section className="workspace-card"><span>NO RESERVATIONS YET</span><h2>Create your first bond.</h2><p>Published reservation bonds will appear here automatically.</p></section>}</div></main></div>
}

function StaffManagement() {
  const profile = storedProfile(); const [members, setMembers] = useState<StaffMember[]>([]); const [wallet, setWallet] = useState(''); const [canRefund, setCanRefund] = useState(false); const [error, setError] = useState('')
  useEffect(() => { if (profile?.role === 'RESTAURANT') getStaff(profile.id).then(value => setMembers(value.members)).catch(caught => setError(caught instanceof Error ? caught.message : 'Could not load staff.')) }, [profile?.id, profile?.role])
  if (!profile || profile.role !== 'RESTAURANT') return <Navigate to="/start?role=restaurant" replace/>
  async function add() { try { await addStaff(profile!.id, wallet, canRefund); setMembers((await getStaff(profile!.id)).members); setWallet('') } catch (caught) { setError(caught instanceof Error ? caught.message : 'Could not add staff.') } }
  async function revoke(id: string) { await revokeStaff(profile!.id, id); setMembers((await getStaff(profile!.id)).members) }
  return <div className="workspace"><header><Mark/><Link to="/restaurant"><X/></Link></header><main><p className="eyebrow">Staff access</p><h1>Approved wallets.</h1><section className="workspace-card"><label>Staff Nimiq address<input value={wallet} onChange={e => setWallet(e.target.value.toUpperCase())}/></label><label className="terms"><input type="checkbox" checked={canRefund} onChange={e => setCanRefund(e.target.checked)}/><span><Check/></span><p>Allow refunds</p></label><button className="button button--app" onClick={() => void add()}>Add staff wallet</button>{error && <div className="error-message">{error}</div>}</section>{members.map(member => <section className="workspace-card" key={member.id}><code>{member.wallet_address}</code><p>{member.can_refund ? 'Check-in and refund permission' : 'Check-in permission'}</p>{member.revoked_at ? <span>REVOKED</span> : <button className="refund-link" onClick={() => void revoke(member.id)}>Revoke access</button>}</section>)}</main></div>
}
...
function Terms() { return <div className="legal"><Mark/><Link to="/"><ArrowLeft/> Back</Link><h1>Terms and limitations.</h1><p>Pactum verifies direct NIM reservation bonds. Pactum never holds funds and is not escrow. Restaurants remain responsible for their stated policies, service decisions, and refunds.</p><p>A transfer changes who may use the reservation; it does not move the original payment. Refunds return funds to the verified original payer through a separate wallet transaction.</p></div> }
...
function App() {
  ...
  return <Routes><Route path="/" element={<Landing/>}/><Route path="/start" element={<Start/>}/><Route path="/guest" element={<GuestHome/>}/><Route path="/restaurant" element={<RestaurantHome/>}/><Route path="/restaurant/new" element={<NewBond/>}/><Route path="/restaurant/staff" element={<StaffManagement/>}/><Route path="/staff/scan" element={<StaffScan/>}/><Route path="/p/:publicId" element={<MiniApp/>}/><Route path="/p/:publicId/transfer" element={<TransferReservation/>}/><Route path="/privacy" element={<Privacy/>}/><Route path="/terms" element={<Terms/>}/><Route path="*" element={<Navigate to="/" replace/>}/></Routes>
}
